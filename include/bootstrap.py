...
def bootstrapping(ents_embedding, ref_pairs, labeled_alignment):
    # sim_mat
    # cotraining se ae各自产生对齐的entity，以下为se
    if ents_embedding is None:
        return
    Lvec = np.array([ents_embedding[e1] for e1, e2 in ref_pairs])
    Rvec = np.array([ents_embedding[e2] for e1, e2 in ref_pairs])
    ref_sim_mat = scipy.spatial.distance.cdist(Lvec, Rvec, metric='cityblock')
    th = Config.th
    ...
    if labeled_alignment is not None:
        ents1 = [ref_pairs[pair[0]] for pair in labeled_alignment]
        ents2 = [ref_pairs[pair[1]] for pair in labeled_alignment]
    else:
